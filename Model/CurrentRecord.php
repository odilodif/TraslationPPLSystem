...
class CurrentRecord implements ISurfMove {
    ...
    public function FielsEmpty() {
        ...
        $query = "SELECT th.trasl_id,	center_crs.name as crs_source,' ' as crs_destination
...
when th.trasl_state_process ='authorized'  then 'Autorizado'
		...
		LEFT JOIN prison_location  center_crs  on th.crs_id_source=center_crs.id
    ...
    LEFT JOIN  traslation_type ty on th.trasl_type_id=ty.trasl_type_id
		...
		LEFT JOIN prison_person pp  on  tdls.prison_per_id=pp.id
		WHERE   th.trasl_id= $this->id  and th.trasl_state='t';";
        return $query;
    }
    ...
    public function move1() {
        $query = "SELECT th.trasl_id,	center_crs.name as crs_source,crsd.name as crs_destination
...
when th.trasl_state_process ='authorized'  then 'Autorizado'
		...
		,pp.id,pp.identificador,pp.name,pp.last_name,pp.id,	pp.sex,	pp.prontuario,	pp.state_boolean
		from traslation_head  th
		INNER JOIN prison_location center_crs  on th.crs_id_source=center_crs.id
    ...
    INNER JOIN  traslation_type ty on th.trasl_type_id=ty.trasl_type_id
		...
		WHERE   th.trasl_id= $this->id  and th.trasl_state='t' and tdls.trasl_det_status='t';";
        return $query;
    }
