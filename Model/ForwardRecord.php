...
when th.trasl_state_process ='AUTHORIZED'  then 'Autorizado'
		...
		WHERE   th.trasl_id=(select MIN(trasl_id) from traslation_head where trasl_id > $this->id and traslation_head.crs_id_source= $this->crs_id and traslation_head.trasl_state='t') and th.trasl_state='t'  and th.crs_id_source=$this->crs_id ;";
        return $query;
    }
...
when th.trasl_state_process ='AUTHORIZED'  then 'Autorizado'
    ...
    INNER JOIN  traslation_type ty on th.trasl_type_id=ty.trasl_type_id
		...
		WHERE   th.trasl_id=(select MIN(trasl_id) from traslation_head where trasl_id > $this->id and traslation_head.crs_id_source= $this->crs_id and traslation_head.trasl_state='t') and th.trasl_state='t' and tdls.trasl_det_status='t'  and th.crs_id_source=$this->crs_id ;";
        return $query;
    }
